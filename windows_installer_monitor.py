...
Single-file utility to monitor what files, registry keys and processes an installer (.exe) creates or modifies.

Features:
- Snapshot mode: take 'before' and 'after' snapshots (filesystem + registry) and compare them.
...
Limitations:
- Needs to be run on Windows with administrator privileges for full registry and system-wide paths.
...
# Try importing Windows-specific modules and optional libs
try:
    import winreg
except Exception:
    winreg = None
...
def snapshot_files(paths, follow_links=False):
...
def compare_file_snapshots(before, after):
    ...
    for p, meta in after.items():
        if p not in before:
            ...
            b = before[p]
            if b['size'] != meta['size'] or int(b['mtime']) != int(meta['mtime']):
...
def read_reg_tree(root, subkey=''):
    """Recursively read registry key and return dict of subkey-> {values, subkeys}
    Only supports Win32 via winreg. Requires appropriate privileges.
    """
    if winreg is None:
    ...
    def _read(handle, path):
        out = {'values': {}, 'subkeys': {}}
        try:
            with winreg.OpenKey(handle, path, 0, winreg.KEY_READ) as key:
                # values
                try:
                    i = 0
                    while True:
                        name, val, typ = winreg.EnumValue(key, i)
                        out['values'][name] = {'value': repr(val), 'type': int(typ)}
                        i += 1
                except OSError:
                    pass
                # subkeys
                try:
                    ...
                    while True:
                        ...
                        out['subkeys'][sub] = _read(handle, path + '\\' + sub if path else sub)
                        i += 1
                except OSError:
                    pass
        except Exception:
            # could be access denied or path not found
            out['__error'] = 'access_denied_or_missing'
        return out

    return {subkey: _read(root_handle, subkey)}


def snapshot_registry(hives=None):
    if winreg is None:
        raise RuntimeError('winreg not available on this platform')
    if hives is None:
        hives = [('HKLM', ''), ('HKCU', '')]
    ...
    for hive, base in hives:
        out[hive] = read_reg_tree(hive, base)
    return out
...
def run_and_snapshot(installer, outdir, paths_to_snapshot=None, wait=5):
    ensure_windows()
    if paths_to_snapshot is None:
        paths_to_snapshot = DEFAULT_WATCH_PATHS
    before_files = snapshot_files(paths_to_snapshot)
    before_reg = snapshot_registry()
    ...
    after_files = snapshot_files(paths_to_snapshot)
    after_reg = snapshot_registry()
    ...
    results = {
        ...
        'registry': {'before': before_reg, 'after': after_reg},
    }
...
def main():
    ...
    if args.cmd == 'run-and-snapshot':
        os.makedirs(args.out, exist_ok=True)
        res = run_and_snapshot(args.installer, args.out, paths_to_snapshot=args.paths, wait=args.wait)
        return
